...
describe("Windows Python AppContainer containment proof", () => {
  ...
  it("launches through an empty-capability AppContainer and a privilege-stripped token", async () => {
    ...
    expect(source).toContain("CreateEnvironmentBlock(&environment, nullptr, FALSE)");
    expect(source).toContain("DestroyEnvironmentBlock(environment)");
    ...
    expect(storage).not.toContain("SetNamedSecurityInfoW");
    expect(storage).not.toContain("SetEntriesInAclW");
