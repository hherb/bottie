...
retained proof environment before any bundle assembly. It runs only on Linux ARM64 with Python 3.12.3, requires the
exact base- and derived-image digests as explicit inputs, verifies every direct Python pin and checked-in proof input,
then records the package-manager inventory. The record contains only component identities, versions, architectures,
package-relative licence labels, exact licence byte counts and hashes, and the measured NVIDIA container-terms bytes.
Absolute paths and licence contents are not retained.
