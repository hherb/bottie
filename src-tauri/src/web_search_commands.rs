...
use crate::{
    ...
    web_search::{
        BRAVE_SEARCH_PROVIDER_ID, BraveSearchProvider, WebSearchError, WebSearchErrorCode,
        WebSearchProvider, connection_test_request,
    },
};

#[tauri::command]
/// Tests the fixed Brave Search route with a draft or saved native-vault credential.
pub(crate) async fn test_web_search_connection(
    ...
    if draft.provider_id != BRAVE_SEARCH_PROVIDER_ID {
    ...
    let api_key = draft
        ...
        .or(state.credentials.get(BRAVE_SEARCH_PROVIDER_ID)?)
        .ok_or_else(|| ProviderError::invalid_request("Enter a Brave Search API key to test."))?;
    let provider = BraveSearchProvider::new(api_key).map_err(map_web_search_error)?;
    if let Err(error) = provider.search(connection_test_request()).await {
        let error = map_web_search_error(error);
        record_diagnostic(
            &state.diagnostics,
            "warn",
            "Web-search connection test failed",
            Some(BRAVE_SEARCH_PROVIDER_ID),
            Some(&error.message),
        )
        .await;
        return Err(error);
    ...
    record_diagnostic(
        ...
        Some(BRAVE_SEARCH_PROVIDER_ID),
        Some(&format!(
            "Fixed Brave Search route responded in {elapsed_ms} ms"
        )),
    )
    .await;
    Ok(WebSearchConnectionTest {
        provider_id: BRAVE_SEARCH_PROVIDER_ID.into(),
        elapsed_ms,
        message: "Connected to Brave Search.".into(),
    })
}
