//! OpenAI-compatible Localmail definition, dispatch, audit, and provider-reuse tests.
...
struct GenerationOpenAiLocalmailExecutor {
    calls: Arc<Mutex<Vec<crate::tool_loop::NativeToolCall>>>,
}

impl NativeLocalmailToolExecutor for GenerationOpenAiLocalmailExecutor {
...
#[test]
fn executes_and_persists_openai_email_with_exact_call_identity() {
    ...
    let localmail = GenerationOpenAiLocalmailExecutor::default();
    ...
    .expect("OpenAI Email round should execute");

    assert_eq!(results[0].tool_call_id, "call_openai_email_1");
...
#[test]
fn rejects_openai_email_when_no_configured_executor_is_present() {
...
fn streams_openai_email_result_and_final_answer_across_two_requests() {
    let (store, _conversation_id, _message_id, run_id) = active_run("openai");
    let tool_event = json!({
        "choices": [{"delta": {"tool_calls": [{
            "index": 0,
            "id": "call_openai_email_1",
            "type": "function",
            "function": {
                "name": "search_email",
    ...
    .expect("two-round OpenAI-compatible Email generation should complete")
