...
mod credentials;
mod diagnostics;
mod generated_asset_preview_protocol;
mod generation;
...
use command_types::{
    AppInfo, ImageGenerationSetupDraft, ImageGenerationSetupStatus, ProviderConnectionDraft,
    ProviderConnectionTest, ProviderCredentialStatus, ProviderCredentialUpdate, ProviderSelection,
};
...
struct AppState {
    ...
    python_runtime: Option<PythonRuntimeState>,
    runs: ActiveRuns,
    image_runs: ImageGenerationRuns,
...
#[tauri::command]
/// Validates the exact Qwen-Image-2.0 route and credential shape without provider I/O.
async fn validate_qwen_image_configuration(
    ...
    record_diagnostic(
        &state.diagnostics,
        "info",
        "Qwen Image setup validated",
        Some(QWEN_IMAGE_PROVIDER_ID),
        Some("Exact Qwen-Image-2.0 checkpoint selected; no billed request sent"),
    )
    .await;
    Ok(status)
}
...
async fn start_microphone_capture(
    ...
    state.image_runs.cancel_active().await;
...
pub fn run() {
    tauri::Builder::default()
        .register_uri_scheme_protocol("bottie-generated-asset", |context, request| {
            ...
            generated_asset_preview_protocol::response(&state.conversations, &request)
        })
        ...
        .setup(|app| {
            ...
            app.manage(AppState {
                ...
                python_approval: Arc::new(PythonApprovalController::with_publisher(
                    app.handle().clone(),
                )),
                python_runtime,
                runs: Arc::new(tauri::async_runtime::Mutex::new(HashMap::new())),
                image_runs: ImageGenerationRuns::default(),
