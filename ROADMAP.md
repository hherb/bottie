...
## Milestone 8 — Generated images

Outcome: users can explicitly generate and edit durable images with exact model provenance through hosted or local
execution chosen by capability, without exposing credentials, provider result URLs, or native paths to the WebView.

Status: active product priority. App-store certification/publication and other shipping work are paused until this
milestone is complete or the product owner explicitly reprioritizes them.

Two model tracks are intentional and must remain visibly distinct:

- **[Qwen-Image-2.0](https://docs.modelstudio.console.alibabacloud.com/en/model-studio/qwen-image-2-0):**
  use the exact hosted `qwen-image-2.0-2026-03-03` snapshot now. Add local execution only when exact 2.0 weights are
  publicly released and independently verified.
- **[Qwen-Image-2512](https://huggingface.co/Qwen/Qwen-Image-2512):** use the currently available Apache-2.0
  `Qwen/Qwen-Image-2512` text-to-image weights for local execution now. Never label 2512 output as Qwen-Image-2.0;
  `2512` means the December 2025 checkpoint.

### 8.1 Shared contract and hosted Qwen-Image-2.0 foundation
...
1. [ ] Add a `generated_assets` migration owned by assistant messages, with content hash, media type, dimensions, byte
   count, created time, provider ID, exact model ID, execution backend, seed when supported, and terminal status.
2. [ ] Add a Rust-owned generation controller with one active run identity, shared cancellation, bounded progress
...
3. [ ] Add an explicit composer action for prompt, aspect ratio, count, execution choice, and provider delivery/cost
   disclosure. Cloud generation must never follow from an ordinary chat send or from a failed local route.
4. [ ] For hosted 2.0, immediately download temporary URLs inside Rust, with redirects disabled, fixed byte/time limits,
...
5. [ ] Present pending, completed, failed, and cancelled assistant image messages; support open, copy, export, retry,
   and deletion without reusing the existing user-attachment association or exposing native paths.
6. [ ] Extend portable export, backup, restore, retention, branching, and garbage collection so generated bytes follow
   the same ownership and recovery guarantees as other conversation assets.
7. [ ] Test malformed provider output, expired URLs, decompression bombs, count/dimension mismatches, cancellation at
...
- [ ] **macOS Apple silicon:** prove a pinned [MLX-Gen](https://github.com/lpalbou/mlx-gen) worker with
  ...
  published packages are roughly 16.2 GiB for mixed 4-bit and 27.5 GiB for 8-bit; do not auto-download either;
